// import React, {useState} from 'react';
// import { Link } from 'react-router-dom';
// import Input from '../../components/input/Input';
// import { useForm ,  useFieldArray } from 'react-hook-form';
// import useAuthHeader from 'react-auth-kit/hooks/useAuthHeader';
// import Textarea from '../../components/textarea/Textarea';
// import SelectInput from '../../components/selectinput/SelectInput';
// import BtnSubmit from '../../components/btnsubmit/BtnSubmit';
// const CreateOeuve: React.FC = () => {
//   const authHeader            = useAuthHeader();
//   interface FormData {
//     name: string;
//     iscreatedat: string;
//     files: { file: FileList | null}[];
//   }
//   const {setValue, control, register, formState: { errors }, handleSubmit } = useForm<FormData>();


  
  
//   const { fields, append, remove } = useFieldArray({
//     control,
//     name: 'files',
//   });
  

//   const [selectedOption, setSelectedOption] = useState<string | number>('');
//   let options = [
//     { value: 'option1', label: 'Option 1' },
//     { value: 'option2', label: 'Option 2' },
//     { value: 'option3', label: 'Option 3' },
//   ];
//   const handleSelectChange = (value: string | number) => {
//     setSelectedOption(value);
//   };
 
//   const [numberOfFile, setNumberOfFile]  = useState(0);
//   const onSubmit = async(data: FormData) => {
//     //recuperation des données
//     // const nameElement = document.getElementById('name');
//     // let named :string ="";
//     // if (nameElement) {
//     //     named= (nameElement as HTMLInputElement).value;
//     // }
//     // const date = document.getElementById('iscreatedat')
//     // let isCreatedAt:string ="";
//     // if(date){
//     //   isCreatedAt = (date as HTMLInputElement).value;
//     // }
//     const desciption = document.getElementById('description')
//     let isDescription:string ="";
//     if(desciption){
//       isDescription = (desciption as HTMLInputElement).value;
//     }
//     const artist = document.getElementById('artist')
//     let isArtist:string ="";
//     if(artist){
//       isArtist = (artist as HTMLInputElement).value;
//     }
//     const formData = new window.FormData();
//     // Ajoutez les fichiers au FormData
//     data.files.forEach((fileWrapper, index) => { 
//       if (fileWrapper && fileWrapper.file && fileWrapper.file.length > 0) {
//         formData.append(`image[${index}]`, fileWrapper.file[0]);
        
//       } else {
//         console.warn(`File at index ${index} is empty or invalid`);
//       } 
//       formData.append("name",data.name )
//       formData.append("isCreatedAt",data.iscreatedat )
//       formData.append("idArtist",isArtist )
//       formData.append("description",isDescription )
//     });
//     //envoye des données au back
//     try{
//       fetch("http://localhost:8889/api/oeuvres/admin/create",{
//         method: "POST",
//         headers:{
//           'Authorization': `${authHeader}` ,
//           // 'Content-Type': 'multipart/form-data',
//         },       
//         body:formData,
//       }).then((res)=>{
//           console.log(res)
//       })
//     }catch(error){
//       console.log(error)
//     }
//   };
//   const createFile =()=>{
//     append({ file: null });
//     setNumberOfFile(numberOfFile +1) 
//   }
//   const removeFile = (index: number)=>{
//     remove(index)
//     setNumberOfFile(numberOfFile -1)
//   }
//   const handleTextareaChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
//     setValue('message', e.target.value);
//   };
//   return (
//     <div className="flex flex-col items-center justify-center h-screen">
//       <h1 className="text-3xl mb-4">la page pour crée une oeuvre</h1>
//       <Link to="/dashboard">
//         <button className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
//           retour dashboard
//         </button>
//       </Link>
//       <form id='form' onSubmit={handleSubmit(onSubmit)} >

      
//       <Input
//         type="text"
//         name="name"
//         label="Nom"
//         errors={errors}
//         register={register}
//         validationSchema={{ 
//           required: true,
//           minLength: 2,
//           maxLength: 50,
//           pattern: { 
//             value: /^[A-Za-z 0-9]{2,50}$/, 
//             message: 'Le format de la catégorie est invalide.' 
//           }
//         }}
//         id='name'
//         value=""  // Par défaut, ajoutez une valeur ou la gestion d'état pour cette valeur.
//         messRequired="Le nom est obligatoire."
//         messMinLength='Le minimum est 2 caractères.'
//         messMaxLength='Le maximum est 50 caractères.'
//         messPattern='Erreur dans le nom'
//         messMax="Valeur trop élevée."
//         messMin="Valeur trop basse."
//         messValidate="Validation incorrecte."
//         onchange={() => {}}  // Vous pouvez définir une fonction pour gérer le changement.
//         container_input='container_mobile'
//         required
//         classe="" 
//         labelcss=""
//       />
//             <Input
//         type="date"
//         name="iscreatedat"
//         label="Date de création"
//         errors={errors}
//         register={register}
//         validationSchema={{ 
//           required: true,
//         }}
//         id='iscreatedat'
//         value=""  // Par défaut, ajoutez une valeur ou la gestion d'état pour cette valeur.
//         messRequired="La date est obligatoire."
//         messMinLength='Le minimum est 2 caractères.'
//         messMaxLength='Le maximum est 50 caractères.'
//         messPattern='Erreur dans le nom'
//         messMax="Valeur trop élevée."
//         messMin="Valeur trop basse."
//         messValidate="Validation incorrecte."
//         onchange={() => {}}  // Vous pouvez définir une fonction pour gérer le changement.
//         container_input='container_mobile'
//         required
//         classe="" 
//         labelcss=""
//       />
//       <SelectInput
//         label="Artiste "
//         options={options}
//         defaultValue={selectedOption}
//         name="artist"
//         validationSchema={{ 
//           required: true,
//         }}
//         register={register}
//       />
//        <Textarea
//         label="Message"
//         onChange={handleTextareaChange}
//         name="message"
//         register={register}
//         validationSchema={{
//           required: 'Ce champ est obligatoire',
//           minLength: {
//             value: 10,
//             message: 'Le message doit contenir au moins 10 caractères',
//           },
//           maxLength: {
//             value: 200,
//             message: 'Le message ne doit pas dépasser 200 caractères',
//           },
//         }}
//         errors={errors}
//         value=""
//         messRequired="Ce champ est obligatoire"
//         messMinLength="Le message est trop court"
//         messMaxLength="Le message est trop long"
//         messPattern=""
//         messMax=""
//         messMin=""
//         messValidate=""
//         labelcss="label-class"
//         classname="textarea-class"
// // Gérer la mise à jour de la valeur
//       /> 
//      <h3> {numberOfFile }/8 images</h3>
//      {numberOfFile<8 && <button type="button" onClick={createFile}>
//       Add File
//     </button>}
// {fields.map((field, index) => (
//         <div key={field.id}>
//           <input type="file" {...register(`files.${index}.file`)} />
//           <button type="button" onClick={()=>{removeFile(index)}}>
//             Remove
//           </button>
//         </div>
//       ))}
      
//       <BtnSubmit container_submit=""
//       click={()=>{}}
//           classe="" id='submit' value='CREER'/>

// </form>
//     </div>
//   );
// };

// export default CreateOeuve;


import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import Input from '../../components/input/Input';
import { useForm, useFieldArray } from 'react-hook-form';
import useAuthHeader from 'react-auth-kit/hooks/useAuthHeader';
import Textarea from '../../components/textarea/Textarea';
import SelectInput from '../../components/selectinput/SelectInput';
import BtnSubmit from '../../components/btnsubmit/BtnSubmit';

const CreateOeuve: React.FC = () => {
  const authHeader = useAuthHeader();

  interface FormData {
    name: string;
    iscreatedat: string;
    files: { file: FileList | null }[];
    message: string; // Add this line
  }

  const { setValue, control, register, formState: { errors }, handleSubmit } = useForm<FormData>();
  const { fields, append, remove } = useFieldArray({
    control,
    name: 'files',
  });

  const [selectedOption, setSelectedOption] = useState<string | number>('');
  const options = [
    { value: 1, label: 'Option 1' },
    { value:2, label: 'Option 2' },
    { value: 3, label: 'Option 3' },
  ];

  const handleSelectChange = (value: string | number) => {
    setSelectedOption(value);
  };

  const [numberOfFile, setNumberOfFile] = useState(0);

  const onSubmit = async (data: FormData) => {

    const artist = document.getElementById('artist');
    let isArtist = "";
    if (artist) {
      isArtist = (artist as HTMLInputElement).value;
    }

    const formData = new window.FormData();
    formData.append("name", data.name);
      formData.append("isCreatedAt", data.iscreatedat);
      formData.append("idArtist", isArtist);
      formData.append("description", data.message);
    data.files.forEach((fileWrapper, index) => {
      if (fileWrapper && fileWrapper.file && fileWrapper.file.length > 0) {
        formData.append(`image`, fileWrapper.file[index]);
      } else {
        console.warn(`File at index ${index} is empty or invalid`);
      }
    });

    try {
      fetch("http://localhost:8889/api/oeuvres/admin/create", {
        method: "POST",
        headers: {
          'Authorization': `${authHeader}`,
        },
        body: formData,
      }).then((res) => {
        console.log(res);
      });
    } catch (error) {
      console.log(error);
    }
  };

  const createFile = () => {
    append({ file: null });
    setNumberOfFile(numberOfFile + 1);
  };

  const removeFile = (index: number) => {
    remove(index);
    setNumberOfFile(numberOfFile - 1);
  };
  const [text, setText] = useState("");
  const handleOnChange =(e:any)=>{
    setText(e.target.value)
  }
  return (
    <div className="flex flex-col items-center justify-center h-screen">
      <h1 className="text-3xl mb-4">la page pour crée une oeuvre</h1>
      <Link to="/dashboard">
        <button className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
          retour dashboard
        </button>
      </Link>
      <form id='form' onSubmit={handleSubmit(onSubmit)} >
        <Input
          type="text"
          name="name"
          label="Nom"
          errors={errors}
          register={register}
          validationSchema={{
            required: true,
            minLength: 2,
            maxLength: 50,
            pattern: {
              value: /^[A-Za-z 0-9]{2,50}$/,
              message: 'Le format de la catégorie est invalide.'
            }
          }}
          id='name'
          value=""
          messRequired="Le nom est obligatoire."
          messMinLength='Le minimum est 2 caractères.'
          messMaxLength='Le maximum est 50 caractères.'
          messPattern='Erreur dans le nom'
          messMax="Valeur trop élevée."
          messMin="Valeur trop basse."
          messValidate="Validation incorrecte."
          onchange={() => { }}
          container_input='container_mobile'
          required
          classe=""
          labelcss=""
        />
        <Input
          type="date"
          name="iscreatedat"
          label="Date de création"
          errors={errors}
          register={register}
          validationSchema={{
            required: true,
          }}
          id='iscreatedat'
          value=""
          messRequired="La date est obligatoire."
          messMinLength='Le minimum est 2 caractères.'
          messMaxLength='Le maximum est 50 caractères.'
          messPattern='Erreur dans le nom'
          messMax="Valeur trop élevée."
          messMin="Valeur trop basse."
          messValidate="Validation incorrecte."
          onchange={() => { }}
          container_input='container_mobile'
          required
          classe=""
          labelcss=""
        />
        <SelectInput
          label="Artiste "
          options={options}
          defaultValue={selectedOption}
          name="artist"
          validationSchema={{
            required: true,
          }}
          register={register}
        />
        <Textarea
          label="Description"
          onChange={handleOnChange }
          name="message"
          register={register}
          validationSchema={{
            required: 'Ce champ est obligatoire',
            minLength: {
              value: 10,
              message: 'Le message doit contenir au moins 10 caractères',
            },
            maxLength: {
              value: 200,
              message: 'Le message ne doit pas dépasser 200 caractères',
            },
          }}
          errors={errors}
          value=""
          messRequired="Ce champ est obligatoire"
          messMinLength="Le message est trop court"
          messMaxLength="Le message est trop long"
          messPattern=""
          messMax=""
          messMin=""
          messValidate=""
          labelcss="label-class"
          classname="textarea-class"

        />
        <h3>{numberOfFile}/8 images</h3>
        {numberOfFile < 8 && <button type="button" onClick={createFile}>
          Add File
        </button>}
        {fields.map((field, index) => (
          <div key={field.id}>
            <input type="file" {...register(`files.${index}.file`)} />
            <button type="button" onClick={() => { removeFile(index) }}>
              Remove
            </button>
          </div>
        ))}
        <BtnSubmit container_submit=""
          click={() => { }}
          classe="" id='submit' value='CREER' />
      </form>
    </div>
  );
};

export default CreateOeuve